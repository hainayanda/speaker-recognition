...
public class Signatures {
	...
	public Signatures(double[] signatures) throws IllegalArgumentException{
		if(signatures == null) throw new IllegalArgumentException("Signatures cannot be null");
	...
	public double getSignature(int index) throws IndexOutOfBoundsException {
		if(index < length() && index >= 0)
			...
			throw new IndexOutOfBoundsException("failed to get signature at index " + index + " because signatures length is " + length());
		else
			throw new IndexOutOfBoundsException("failed to get signature at index " + index + " because index cannot be negative");
	}
