...
public class AudioRecorder {
	...
	public static Sound record(float sampleRate, int bitDepth, int length) throws IllegalArgumentException{
		if(bitDepth%8 != 0) throw new IllegalArgumentException("bitDepth parameter must be in byte size, you are trying to input " + bitDepth + " as parameter, which is incorrect");
		else if(length <= 0) throw new IllegalArgumentException("length parameter must be greater than 0, you are trying to input " + bitDepth + " as parameter, which is incorrect");
